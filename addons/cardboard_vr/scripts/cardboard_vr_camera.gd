...
# --- NUEVA CATEGORÍA PARA EL MOVIMIENTO WASD ---
...
# --- NUEVO MÉTODO PARA EL MOVIMIENTO FÍSICO ---
func _physics_process(_delta: float) -> void:
	if not Active or parent == null:
		return

	# Obtener el vector de entrada usando tus acciones de mapeo (WASD)
	var input_dir := Input.get_vector("left", "right", "up", "down")
	
	# Calculamos la dirección del movimiento basándonos en hacia dónde mira el cuerpo del padre
	...
	# Aplicar el movimiento al CharacterBody3D
